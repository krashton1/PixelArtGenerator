...
void TreeGenerator::buildTree()
{
	...
	int numBranches = rand() % (mMaxBranch + 1 - mMinBranch) + mMinBranch;
	...
	for (int i = 0; i < numNodes; i++)
	{
		...
		int thisWidth = floor(maxWidth - (mTaperTrunk ? (float(i) / numNodes) * maxWidth : 0));
	...
	if (!mTaperTrunk)
		sprayPixel((Vector2(0, 0) + mNodes[(mNodes.size() - 1)] + mNodes[(mNodes.size() - 2)]) / 2.0, maxWidth / 2, 1.0, mTrunkColorRamp[1], false);
	...
	for (int i = 0; i < numBranches; i++)
	{
		...
		while (buildOffNode == 0)
		{
			...
			if (mMirrorBranches || !mTaperTrunk)
			{
			...
			{
				if (t % 2 == 0 && mMaxBranch == 2)
				{

					usedLeft = true;
					usedHeight = floor(t / 2);
				}
				else if(mMaxBranch == 2)
				{
...
branchEnd:;
	...
	for (int i = 0; i < mLeaves.size(); i++)
	{
		...
		sprayPixel(currentLeaf, floor(size * 2.0), 0.7 * mLeafDensity, mLeafColorRamp[0]);
		sprayPixel(currentLeaf, floor(size * 1.5), 0.6 * mLeafDensity, mLeafColorRamp[0]);
		sprayPixel(currentLeaf, floor(size * 1.0), 0.6 * mLeafDensity, mLeafColorRamp[0]);
		sprayPixel(currentLeaf + Vector2(0, 0), floor(size * 2.0), 0.2 * mLeafDensity, mLeafColorRamp[1], true);
		sprayPixel(currentLeaf + Vector2(-2, -2), floor(size * 1.7), 0.6 * mLeafDensity, mLeafColorRamp[1]);
		sprayPixel(currentLeaf + Vector2(-4, -4), floor(size * 1.7), 0.4 * mLeafDensity, mLeafColorRamp[2], true);
